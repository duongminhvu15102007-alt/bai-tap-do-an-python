...
gameover_sfx = pygame.mixer.Sound("gameover.wav")
youwin_sfx   = pygame.mixer.Sound("youwin.wav")
...
# ── Trang thai toan cuc ───────────────────────────────
STATE = "menu"   # "menu" | "play" | "youwin" | "gameover"
...
def draw_menu():
    ...
    draw_button("X  Thoat game",   BTN_QUIT0, BTN_QUIT0.collidepoint(mx, my), red=True)
    pygame.display.flip()

def draw_endscreen(win, score):
    """Tra ve 'replay' | 'quit' | None."""
    screen.blit(bg, (0, 0)); overlay()
    mx, my = pygame.mouse.get_pos()
...
while True:
    ...
    if alive:
        ...
        if keys[pygame.K_UP]    and pl.top   > H // 2: pl.y -= P_SPD
        if keys[pygame.K_DOWN]  and pl.bottom < H:      pl.y += P_SPD
    ...
    for e in ene[:]:
        for b in bul[:]:
            if e.colliderect(b):
                ...
                explode_sfx.play(); gs["score"] += 1; break

    # Dan player vs boss
    if bs["rect"]:
        for b in bul[:]:
            if bs["rect"].colliderect(b):
                bul.remove(b); bs["hp"] -= 1; explode_sfx.play()
        if bs["hp"] <= 0:
    ...
    # ── Phat nhac ket thuc dung 1 lan ─────────────────
    if (gs["you_win"] or gs["gameover"]) and not gs["sfx_played"]:
        pygame.mixer.stop()
        sfx = youwin_sfx if gs["you_win"] else gameover_sfx
        sfx.play()
        gs["sfx_played"] = True
        # Doi nhac phat xong roi moi hien man hinh ket thuc
        pygame.time.wait(int(sfx.get_length() * 1000))
